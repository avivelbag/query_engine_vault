...
def test_parser_single_column():
    ...
    assert ast == {"type": "select", "columns": ["id"], "from": "employees", "where": None}


def test_parser_two_columns():
    ast = parse("SELECT id, name FROM employees")
    assert ast == {"type": "select", "columns": ["id", "name"], "from": "employees", "where": None}


def test_parser_three_columns():
    ast = parse("SELECT id, name, department FROM employees;")
    assert ast == {"type": "select", "columns": ["id", "name", "department"], "from": "employees", "where": None}
