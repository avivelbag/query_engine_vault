...
def test_parser_single_column():
    ...
    assert ast == {"type": "select", "distinct": False, "columns": ["id"], "from": "employees", "join": None, "where": None, "group_by": [], "having": None, "order_by": [], "limit": None}


def test_parser_two_columns():
    ast = parse("SELECT id, name FROM employees")
    assert ast == {"type": "select", "distinct": False, "columns": ["id", "name"], "from": "employees", "join": None, "where": None, "group_by": [], "having": None, "order_by": [], "limit": None}


def test_parser_three_columns():
    ast = parse("SELECT id, name, department FROM employees;")
    assert ast == {"type": "select", "distinct": False, "columns": ["id", "name", "department"], "from": "employees", "join": None, "where": None, "group_by": [], "having": None, "order_by": [], "limit": None}
